...
class GraphsTuple(tuple, Generic[_State, _EnvState]):
    ...
    xg: Float[Array, "sum_n_node n_xg"] = None  # global features
    ...
    def __new__(
        ...
        tup = (n_node, n_edge, nodes, edges, states, receivers, senders, node_type, env_states, connectivity, xg)
    ...
    def to_tuple(self):
        return (self.n_node, self.n_edge, self.nodes, self.edges, self.states, self.receivers, self.senders,
                self.node_type, self.env_states, self.connectivity, self.xg)
...
class GetGraph(NamedTuple):
    ...
    xg: Float[Array, "n_node n_xg"] = None  # global features
