...
class MultiAgentPolicy(ABC):
    ...
    def __init__(self, node_dim: int, edge_dim: int, n_agents: int, action_dim: int, dim_factor: int = 2):
...
class DeterministicPolicy(MultiAgentPolicy):
    ...
    def __init__(
        ...
        super().__init__(node_dim, edge_dim, n_agents, action_dim, dim_factor)
    ...
    def get_action(self, params: Params, obs: GraphsTuple) -> Action:
        assert isinstance(params, dict), "Expected dict but got {}".format(type(params))
