...
def get_leader_id_dir(graph: GraphsTuple) -> [Array, Array]:
    """
...
def inside_obstacles(points: Pos, obstacles: Obstacle, r: Radius = 0.) -> BoolScalar:
    ...
    if points.ndim == 1:
        ...
        if obstacles.center.ndim == 1:
            is_in = jax.vmap(ft.partial(inside, obstacle=obstacles))(points)
        else:
